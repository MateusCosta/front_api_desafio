...
<td align="center">{{props.item.id}}</td>
         ...
         <td align="center">
          ...
          <v-divider class="pa-1"></v-divider>
          <v-btn slot="activator"  dark round color="blue" small  @click="desativarProdutos()">Desativar </v-btn>
            </v-card-text>
          </v-card>
        </v-flex>  
      </v-layout>
...
export default {
  ...
  methods: {
    ...
    toggleAll () {
        ...
        else this.selected = this.itemsProdutos.slice()
      },
      changeSort (column) {
      ...
      desativarProdutos () {
        ...
        api.desativar(this.selected).then(ret => {
    ...
    formatFilterData: function (data_fim_nascimento) {
      
      if (!data_fim_nascimento) return ''
      data_fim_nascimento = data_fim_nascimento.toString()
      data_fim_nascimento = data_fim_nascimento.replace('T00:00:00.000Z', '')
      const [year, month, day] = data_fim_nascimento.split('-')
